...
>
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				<title>
					<xsl:choose>
						<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">Mapa witryny  –</xsl:when>
						<xsl:otherwise>Indeks mapy witryny</xsl:otherwise>
					</xsl:choose>
				</title>
		...
		.breadcrumb svg.back {
			...
			<body>
				...
				<xsl:call-template name="Header">
					<xsl:with-param name="title">Mapa witryny  –</xsl:with-param>
	...
	<a href="http://dzikikoder.github.io/Electronic-Store---Affordable-and-Quality-Electronics"><span>Strona główna</span></a>
			...
			<table cellpadding="3">
				...
				<tr>
					<th class="left">
						Adres URL					</th>
					<th>Liczba adresów URL</th>
					<th>
						Ostatnia aktualizacja					</th>
				</tr>
				</thead>
				<tbody>
	...
	<a href="http://dzikikoder.github.io/Electronic-Store---Affordable-and-Quality-Electronics"><span>Strona główna</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="http://dzikikoder.github.io/Electronic-Store---Affordable-and-Quality-Electronics/sitemap.xml"><span>Indeks mapy witryny</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
	...
	<div id="content-head">
		...
		<p>To jest wygenerowana za pomocą <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, mapa witryny XML, przeznaczona dla takich wyszukiwarek jak Google czy Bing.</p>
		<p>
			Więcej informacji na temat map witryn XML można znaleźć na stronie <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Ta mapa witryny zawiera						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								Adres URL							</xsl:when>
							<xsl:otherwise>
								Adresy URL							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
						Ten indeks map witryny zawiera						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								mapa witryny							</xsl:when>
							<xsl:otherwise>
								mapy witryn							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				— wygenerowano: 2023-06-13 o 19:47			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">Niski</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">Średnie</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">Wysoki</xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<a href="http://dzikikoder.github.io/Electronic-Store---Affordable-and-Quality-Electronics"><span>Strona główna</span></a>
		...
		<h2 class="empty-sitemap__title">
			Ups!			<br />
			Tutaj nie ma wpisów		</h2>
		<div class="empty-sitemap__buttons">
			<a href="http://dzikikoder.github.io/Electronic-Store---Affordable-and-Quality-Electronics" class="button">Powrót do strony głównej</a>
					</div>
